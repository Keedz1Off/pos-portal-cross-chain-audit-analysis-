...
|The contract receives "garbage" (invalid data type), but instead of throwing an error (revert), it completes the transaction with the Success status.  |Silent Failure| if..., else if..., else revert(""ChildChainManager: INVALID_SYNC_TYPE"")  |
